...
class AsyncWebSocketClient:
    ...
    def is_connected(self) -> bool:
        return (
            ...
            and getattr(self.websocket, "open", self.websocket_is_connected)
    ...
    async def connect(self, urls: List[str], ssid: str) -> bool:
        logger.debug("--- PROTOTYPE VERSION 3 (ADAPTIVE) ACTIVE ---")
        for url in urls:
            try:
                ...
                header_params = ["additional_headers", "extra_headers", "headers"]
                last_error = None
                connected = False
                
                for param in header_params:
                    try:
                        connect_kwargs = base_kwargs.copy()
                        connect_kwargs[param] = DEFAULT_HEADERS
                        
                        self.websocket = await asyncio.wait_for(
                            websockets.connect(url, **connect_kwargs),
                            timeout=CONNECTION_SETTINGS.get("handshake_timeout", 10.0)
                        )
                        logger.success(f"✅ Connected to WebSocket using parameter: {param}")
                        connected = True
                        break
                    except TypeError:
                        logger.debug(f"Parameter '{param}' not supported by websockets version, trying next...")
                        continue
                ...
                if not connected:
                    raise last_error or ConnectionError(f"Failed to connect to {url} with any header parameter")
    ...
    async def send_message(self, message: str) -> None:
        ...
        """
        is_closed = getattr(self.websocket, "closed", not self.websocket_is_connected)
    ...
    async def _send_handshake(self, ssid: str) -> None:
        try:
            ...
            handshake = ssid
            if not ssid.startswith('42["authorization"'):
                # Wrap it if it's just a raw session ID
                handshake = f'42["authorization",{{ "session": "{ssid}" }}]'
                logger.debug(f"Wrapped session ID in authorization frame: {handshake[:50]}...")
            else:
                logger.debug("Using raw authorization frame for handshake")
                
            await self.send_message(handshake)
            logger.debug(f"Sent authorization message: {handshake[:100]}...")
    ...
    async def _ping_loop(self) -> None:
        while self._running and self.websocket:
            try:
                ...
                # Engine.IO ping is '2'
                if self.websocket and getattr(self.websocket, "open", self.websocket_is_connected):
